...
function createFileManagerStore() {
    ...
    const visibleDesktopFiles = $derived.by(() => {
        let list = showHidden
            ? desktop.files
            : desktop.files.filter((f) => !f.name.startsWith('.'));
    ...
    const visibleAdbFiles = $derived.by(() => {
        let list = showHidden
            ? adb.files
            : adb.files.filter((f) => !f.name.startsWith('.'));

        if (adb.meta.parent) {
            list = [
                { name: '..', path: adb.meta.parent, is_dir: true, has_permission: true },
                ...list
            ];
        }
        return list;
    ...
    async function openFile(file: FileItem, type: 'desktop' | 'adb') {
        ...
        try {
            if (type === 'desktop') {
                ...
                await invoke('adb_pull', { serial: adb.serial, src: file.path, dest: tempPath, isDir: false });
    ...
    async function transferFile(file: FileItem, type: 'desktop' | 'adb') {
        ...
        try {
            if (type === 'desktop') {
                await invoke('adb_push', { serial: adb.serial, src: file.path, dest: adb.path, isDir: file.is_dir });
                await navigateAdb(adb.path);
                await invoke('notify', { body: `Pushed ${file.name} to device.` });
            } else {
                await invoke('adb_pull', { serial: adb.serial, src: file.path, dest: desktop.path, isDir: file.is_dir });
                await navigateDesktop(desktop.path);
    ...
    return {
        ...
        set showHidden(v: boolean) { showHidden = v; },
        get partitions() { return partitions },
        get partitionHistory() { return partitionHistory },
        get activeDevice() { return activeDevice },
        get visibleDesktopFiles() { return visibleDesktopFiles },
        get visibleAdbFiles() { return visibleAdbFiles },
        get partLabel() { return partLabel },
        navigateDesktop,
        navigateAdb,
        openFile,
        deleteFile,
